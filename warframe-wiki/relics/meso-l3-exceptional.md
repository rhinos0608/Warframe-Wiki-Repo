---
acquisition:
- Another Betrayer, Rotation B
- Ceres/Seimeni (Defense), Rotation B
- Earth/Coba (Defense), Rotation B
- Eris/Akkad (Defense), Rotation B
- Europa/Larzac (Defense), Rotation B
- Family Reunion, Rotation B
- Hot Mess, Rotation B
- Jupiter/Sinai (Defense), Rotation B
- Mars/Kadesh (Defense), Rotation B
- Phobos/Opik (Rush), Rotation B
- Pluto/Sechura (Defense), Rotation B
- Recover The Orokin Archive, Rotation B
- Sedna/Sangeru (Defense), Rotation B
- Sunkiller, Rotation B
- Table For Two, Rotation B
- The Aftermath, Rotation B
- Time's Up, Rotation B
- Venus/Romula (Defense), Rotation B
- Saturn/Kasio's Rest (Skirmish)
- Saturn/Lupal Pass (Skirmish)
- Saturn/Mordo Cluster (Skirmish)
- Saturn/Nodo Gap (Skirmish)
- Saturn/Vand Cluster (Skirmish)
- Void/Ani (Survival), Rotation A
- Void/Taranis (Defense), Rotation B
- Neptune/Nu-Gua Mines (Skirmish)
- Neptune/Sovereign Grasp (Skirmish)
- Void Storm (Neptune)
- Void Storm (Saturn)
- Zariman/Halako Perimeter (Exterminate)
- Zariman/The Greenway (Mobile Defense)
- Deimos/Effervo (Assassination)
- Deimos/Nex (Exterminate)
- Zariman/Everview Arc (Void Flood), Rotation A
- Zariman/Oro Works (Void Armageddon), Rotation A
- Zariman/Tuvul Commons (Void Cascade), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation B
- Deimos/Cambion Drift (Level 50 - 60 Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Isolation Vault), Rotation B
- Deimos/Persto (Survival), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation B
- Neptune/Arva Vector (Caches), Rotation A
- Neptune/Brom Cluster (Caches), Rotation A
- Neptune/Enkidu Ice Drifts (Caches), Rotation A
- Neptune/Mammon's Prospect (Caches), Rotation A
- Neptune/Nu-Gua Mines (Caches), Rotation A
- Neptune/Sovereign Grasp (Caches), Rotation A
- Pluto/Fenton's Field (Caches), Rotation A
- Pluto/Khufu Envoy (Caches), Rotation A
- Pluto/Obol Crossing (Caches), Rotation A
- Pluto/Peregrine Axis (Caches), Rotation A
- Pluto/Profit Margin (Caches), Rotation A
- Pluto/Seven Sirens (Caches), Rotation A
- Neptune/Arva Vector (Skirmish), Rotation C
- Neptune/Mammon's Prospect (Skirmish), Rotation B
- Saturn/Pandora (Pursuit)
- Deimos/Cambire (Alchemy), Rotation A
- Deimos/Munio (Defense), Rotation A
- Höllvania/Antivirus Bounty (Exterminate)
- 'Höllvania/Exterminate: Scaldra (Exterminate)'
- 'Höllvania/Exterminate: Techrot (Exterminate)'
- Ceres/Cinxia (Interception), Rotation B
- Earth/Gaia (Interception), Rotation B
- Jupiter/Ganymede (Disruption), Rotation A
- Mars/Alator (Interception), Rotation B
- Mercury/Odin (Interception), Rotation B
- Venus/Cytherean (Interception), Rotation B
- Saturn/Kasio's Rest (Caches)
- Saturn/Lupal Pass (Caches)
- Saturn/Mordo Cluster (Caches)
- Saturn/Nodo Gap (Caches)
- Saturn/Vand Cluster (Caches)
- Void/Ani (Survival), Rotation C
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation B
- Neptune/Enkidu Ice Drifts (Skirmish), Rotation C
- Void/Oxomoco (Exterminate)
- Void/Stribog (Sabotage)
- Void/Taranis (Defense), Rotation C
- Void/Tiwaz (Mobile Defense)
- Void/Ukko (Capture)
- Earth/Saya's Visions (Shrine Defense)
- Void/Belenus (Defense), Rotation A
- Ceres/Draco (Survival), Rotation B
- Europa/Lillith (Excavation), Rotation B
- Europa/Valefor (Excavation), Rotation B
- Europa/Zagan (Survival), Rotation B
- 'Höllvania/Assassinate: H-09 Tank (Assassination)'
- Jupiter/Cameria (Survival), Rotation B
- Jupiter/Elara (Survival), Rotation B
- Mars/Wahiba (Survival), Rotation B
- Phobos/Zeugma (Survival), Rotation B
- Saturn/Piscinas (Survival), Rotation B
- Saturn/Titan (Survival), Rotation B
- Ceres/Seimeni (Defense), Rotation C
- Earth/Coba (Defense), Rotation C
- Eris/Akkad (Defense), Rotation C
- Europa/Larzac (Defense), Rotation C
- Jupiter/Sinai (Defense), Rotation C
- Mars/Kadesh (Defense), Rotation C
- Pluto/Sechura (Defense), Rotation C
- Sedna/Sangeru (Defense), Rotation C
- Venus/Romula (Defense), Rotation C
- Uranus/Brutus (Ascension)
- 'Höllvania/Hell-Scrub: Scaldra (Survival), Rotation A'
- 'Höllvania/Hell-Scrub: Techrot (Survival), Rotation A'
- Another Betrayer, Rotation C
- Family Reunion, Rotation C
- Hot Mess, Rotation C
- Phobos/Opik (Rush), Rotation C
- Recover The Orokin Archive, Rotation C
- Sunkiller, Rotation C
- Table For Two, Rotation C
- The Aftermath, Rotation C
- Time's Up, Rotation C
- Ceres/Casta (Defense), Rotation B
- Ceres/Casta (Defense), Rotation C
- Ceres/Egeria (Defense), Rotation B
- Ceres/Egeria (Defense), Rotation C
- Ceres/Varro (Defense), Rotation B
- Ceres/Varro (Defense), Rotation C
- Earth/Lith (Defense), Rotation B
- Earth/Lith (Defense), Rotation C
- Mars/Spear (Defense), Rotation B
- Mars/Spear (Defense), Rotation C
- Mercury/Lares (Defense), Rotation B
- Mercury/Lares (Defense), Rotation C
- Phobos/Drunlo (Defense), Rotation B
- Phobos/Drunlo (Defense), Rotation C
- Phobos/Gulliver (Defense), Rotation B
- Phobos/Gulliver (Defense), Rotation C
- Venus/Tessera (Defense), Rotation B
- Venus/Tessera (Defense), Rotation C
- 'Faceoff: Single Squad (Steel Path), Rotation A'
- 'Faceoff: Single Squad, Rotation A'
- 'Faceoff: Squad VS Squad (Steel Path), Rotation A'
- 'Faceoff: Squad VS Squad, Rotation A'
- Sanctuary Onslaught, Rotation B
- Deimos/Armatus (Disruption), Rotation A
- Europa/Ose (Interception), Rotation A
- Jupiter/Callisto (Interception), Rotation A
- Neptune/Brom Cluster (Skirmish), Rotation B
- Saturn/Caracol (Defection), Rotation B
- Saturn/Rhea (Interception), Rotation A
- Uranus/Umbriel (Interception), Rotation A
- Ceres/Bode (Spy), Rotation C
- Deimos/Hyf (Defense), Rotation C
- Eris/Cyath (Spy), Rotation B
- Eris/Gnathos (Spy), Rotation B
- Europa/Valac (Spy), Rotation C
- Jupiter/Amalthea (Spy), Rotation C
- Kuva Fortress/Pago (Spy), Rotation C
- Neptune/Nereid (Spy), Rotation B
- Pluto/Oceanum (Spy), Rotation B
- Saturn/Aegaeon (Spy), Rotation C
- Saturn/Dione (Spy), Rotation C
- Sedna/Kelpie (Spy), Rotation B
- Sedna/Tikoloshe (Spy), Rotation B
- Uranus/Rosalind (Spy), Rotation B
- Void/Ani (Survival), Rotation B
- Höllvania/Solstice Square (Defense), Rotation A
- Höllvania/Legacyte Harvest (Legacyte Harvest), Rotation A
- 'Höllvania/Hell-Scrub: Scaldra (Survival), Rotation C'
- 'Höllvania/Hell-Scrub: Techrot (Survival), Rotation C'
- Ceres/Cinxia (Interception), Rotation C
- Earth/Gaia (Interception), Rotation C
- Mars/Alator (Interception), Rotation C
- Mercury/Odin (Interception), Rotation C
- Venus/Cytherean (Interception), Rotation C
- Europa/Paimon (Defense), Rotation A
- Jupiter/Io (Defense), Rotation A
- Saturn/Helene (Defense), Rotation A
- Sedna/Camenae (Defense), Rotation A
- Mars/Olympus (Disruption), Rotation C
- Sedna/Kappa (Disruption), Rotation A
- Uranus/Ur (Disruption), Rotation A
- Höllvania/Legacyte Harvest (Legacyte Harvest), Rotation C
category: Relics
description: An artifact containing Orokin secrets. It can only be opened through
  the power of the Void.
image: ../images/meso-exceptional.png
last_updated: '2025-09-17'
name: Meso L3 Exceptional
rewards:
- chance: 2
  item:
    name: Lavos Prime Systems Blueprint
    uniqueName: /Lotus/Types/Game/Projections/T2VoidProjectionCalibanPrimeABronze
    warframeMarket:
      id: 67acd945125398d92c8bb4ee
      urlName: lavos_prime_systems_blueprint
  rarity: Rare
- chance: 11
  item:
    name: Okina Prime Handle
    uniqueName: /Lotus/Types/Game/Projections/T2VoidProjectionCalibanPrimeABronze
    warframeMarket:
      id: 663268d1e85cac3856c86dbf
      urlName: okina_prime_handle
  rarity: Uncommon
- chance: 11
  item:
    name: Acceltra Prime Receiver
    uniqueName: /Lotus/Types/Game/Projections/T2VoidProjectionCalibanPrimeABronze
    warframeMarket:
      id: 65a7fcc4ed5b0d2e3eb462d1
      urlName: acceltra_prime_receiver
  rarity: Uncommon
- chance: 25.33
  item:
    name: Paris Prime Upper Limb
    uniqueName: /Lotus/Types/Game/Projections/T2VoidProjectionCalibanPrimeABronze
    warframeMarket:
      id: 54a73e65e779893a797fff57
      urlName: paris_prime_upper_limb
  rarity: Uncommon
- chance: 25.33
  item:
    name: Forma Blueprint
    uniqueName: /Lotus/Types/Game/Projections/T2VoidProjectionCalibanPrimeABronze
  rarity: Uncommon
- chance: 25.33
  item:
    name: Sevagoth Prime Neuroptics Blueprint
    uniqueName: /Lotus/Types/Game/Projections/T2VoidProjectionCalibanPrimeABronze
    warframeMarket:
      id: 66c6080366bbb2aedca6957d
      urlName: sevagoth_prime_neuroptics_blueprint
  rarity: Uncommon
source: WFCD
tags:
- Relic
- Relics
tradable: true
type: Relic
---

# Meso L3 Exceptional

An artifact containing Orokin secrets. It can only be opened through the power of the Void.

## Relic Information

## Rewards

| Item | Rarity | Chance |
|------|---------|--------|
| Unknown | Rare | 2 |
| Unknown | Uncommon | 11 |
| Unknown | Uncommon | 11 |
| Unknown | Uncommon | 25.33 |
| Unknown | Uncommon | 25.33 |
| Unknown | Uncommon | 25.33 |

## Acquisition

Available from:
- Another Betrayer, Rotation B
- Ceres/Seimeni (Defense), Rotation B
- Earth/Coba (Defense), Rotation B
- Eris/Akkad (Defense), Rotation B
- Europa/Larzac (Defense), Rotation B
- Family Reunion, Rotation B
- Hot Mess, Rotation B
- Jupiter/Sinai (Defense), Rotation B
- Mars/Kadesh (Defense), Rotation B
- Phobos/Opik (Rush), Rotation B
- Pluto/Sechura (Defense), Rotation B
- Recover The Orokin Archive, Rotation B
- Sedna/Sangeru (Defense), Rotation B
- Sunkiller, Rotation B
- Table For Two, Rotation B
- The Aftermath, Rotation B
- Time's Up, Rotation B
- Venus/Romula (Defense), Rotation B
- Saturn/Kasio's Rest (Skirmish)
- Saturn/Lupal Pass (Skirmish)
- Saturn/Mordo Cluster (Skirmish)
- Saturn/Nodo Gap (Skirmish)
- Saturn/Vand Cluster (Skirmish)
- Void/Ani (Survival), Rotation A
- Void/Taranis (Defense), Rotation B
- Neptune/Nu-Gua Mines (Skirmish)
- Neptune/Sovereign Grasp (Skirmish)
- Void Storm (Neptune)
- Void Storm (Saturn)
- Zariman/Halako Perimeter (Exterminate)
- Zariman/The Greenway (Mobile Defense)
- Deimos/Effervo (Assassination)
- Deimos/Nex (Exterminate)
- Zariman/Everview Arc (Void Flood), Rotation A
- Zariman/Oro Works (Void Armageddon), Rotation A
- Zariman/Tuvul Commons (Void Cascade), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation B
- Deimos/Cambion Drift (Level 50 - 60 Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Isolation Vault), Rotation B
- Deimos/Persto (Survival), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation B
- Neptune/Arva Vector (Caches), Rotation A
- Neptune/Brom Cluster (Caches), Rotation A
- Neptune/Enkidu Ice Drifts (Caches), Rotation A
- Neptune/Mammon's Prospect (Caches), Rotation A
- Neptune/Nu-Gua Mines (Caches), Rotation A
- Neptune/Sovereign Grasp (Caches), Rotation A
- Pluto/Fenton's Field (Caches), Rotation A
- Pluto/Khufu Envoy (Caches), Rotation A
- Pluto/Obol Crossing (Caches), Rotation A
- Pluto/Peregrine Axis (Caches), Rotation A
- Pluto/Profit Margin (Caches), Rotation A
- Pluto/Seven Sirens (Caches), Rotation A
- Neptune/Arva Vector (Skirmish), Rotation C
- Neptune/Mammon's Prospect (Skirmish), Rotation B
- Saturn/Pandora (Pursuit)
- Deimos/Cambire (Alchemy), Rotation A
- Deimos/Munio (Defense), Rotation A
- Höllvania/Antivirus Bounty (Exterminate)
- Höllvania/Exterminate: Scaldra (Exterminate)
- Höllvania/Exterminate: Techrot (Exterminate)
- Ceres/Cinxia (Interception), Rotation B
- Earth/Gaia (Interception), Rotation B
- Jupiter/Ganymede (Disruption), Rotation A
- Mars/Alator (Interception), Rotation B
- Mercury/Odin (Interception), Rotation B
- Venus/Cytherean (Interception), Rotation B
- Saturn/Kasio's Rest (Caches)
- Saturn/Lupal Pass (Caches)
- Saturn/Mordo Cluster (Caches)
- Saturn/Nodo Gap (Caches)
- Saturn/Vand Cluster (Caches)
- Void/Ani (Survival), Rotation C
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation A
- Deimos/Cambion Drift (Level 50 - 60 Arcana Isolation Vault), Rotation B
- Neptune/Enkidu Ice Drifts (Skirmish), Rotation C
- Void/Oxomoco (Exterminate)
- Void/Stribog (Sabotage)
- Void/Taranis (Defense), Rotation C
- Void/Tiwaz (Mobile Defense)
- Void/Ukko (Capture)
- Earth/Saya's Visions (Shrine Defense)
- Void/Belenus (Defense), Rotation A
- Ceres/Draco (Survival), Rotation B
- Europa/Lillith (Excavation), Rotation B
- Europa/Valefor (Excavation), Rotation B
- Europa/Zagan (Survival), Rotation B
- Höllvania/Assassinate: H-09 Tank (Assassination)
- Jupiter/Cameria (Survival), Rotation B
- Jupiter/Elara (Survival), Rotation B
- Mars/Wahiba (Survival), Rotation B
- Phobos/Zeugma (Survival), Rotation B
- Saturn/Piscinas (Survival), Rotation B
- Saturn/Titan (Survival), Rotation B
- Ceres/Seimeni (Defense), Rotation C
- Earth/Coba (Defense), Rotation C
- Eris/Akkad (Defense), Rotation C
- Europa/Larzac (Defense), Rotation C
- Jupiter/Sinai (Defense), Rotation C
- Mars/Kadesh (Defense), Rotation C
- Pluto/Sechura (Defense), Rotation C
- Sedna/Sangeru (Defense), Rotation C
- Venus/Romula (Defense), Rotation C
- Uranus/Brutus (Ascension)
- Höllvania/Hell-Scrub: Scaldra (Survival), Rotation A
- Höllvania/Hell-Scrub: Techrot (Survival), Rotation A
- Another Betrayer, Rotation C
- Family Reunion, Rotation C
- Hot Mess, Rotation C
- Phobos/Opik (Rush), Rotation C
- Recover The Orokin Archive, Rotation C
- Sunkiller, Rotation C
- Table For Two, Rotation C
- The Aftermath, Rotation C
- Time's Up, Rotation C
- Ceres/Casta (Defense), Rotation B
- Ceres/Casta (Defense), Rotation C
- Ceres/Egeria (Defense), Rotation B
- Ceres/Egeria (Defense), Rotation C
- Ceres/Varro (Defense), Rotation B
- Ceres/Varro (Defense), Rotation C
- Earth/Lith (Defense), Rotation B
- Earth/Lith (Defense), Rotation C
- Mars/Spear (Defense), Rotation B
- Mars/Spear (Defense), Rotation C
- Mercury/Lares (Defense), Rotation B
- Mercury/Lares (Defense), Rotation C
- Phobos/Drunlo (Defense), Rotation B
- Phobos/Drunlo (Defense), Rotation C
- Phobos/Gulliver (Defense), Rotation B
- Phobos/Gulliver (Defense), Rotation C
- Venus/Tessera (Defense), Rotation B
- Venus/Tessera (Defense), Rotation C
- Faceoff: Single Squad (Steel Path), Rotation A
- Faceoff: Single Squad, Rotation A
- Faceoff: Squad VS Squad (Steel Path), Rotation A
- Faceoff: Squad VS Squad, Rotation A
- Sanctuary Onslaught, Rotation B
- Deimos/Armatus (Disruption), Rotation A
- Europa/Ose (Interception), Rotation A
- Jupiter/Callisto (Interception), Rotation A
- Neptune/Brom Cluster (Skirmish), Rotation B
- Saturn/Caracol (Defection), Rotation B
- Saturn/Rhea (Interception), Rotation A
- Uranus/Umbriel (Interception), Rotation A
- Ceres/Bode (Spy), Rotation C
- Deimos/Hyf (Defense), Rotation C
- Eris/Cyath (Spy), Rotation B
- Eris/Gnathos (Spy), Rotation B
- Europa/Valac (Spy), Rotation C
- Jupiter/Amalthea (Spy), Rotation C
- Kuva Fortress/Pago (Spy), Rotation C
- Neptune/Nereid (Spy), Rotation B
- Pluto/Oceanum (Spy), Rotation B
- Saturn/Aegaeon (Spy), Rotation C
- Saturn/Dione (Spy), Rotation C
- Sedna/Kelpie (Spy), Rotation B
- Sedna/Tikoloshe (Spy), Rotation B
- Uranus/Rosalind (Spy), Rotation B
- Void/Ani (Survival), Rotation B
- Höllvania/Solstice Square (Defense), Rotation A
- Höllvania/Legacyte Harvest (Legacyte Harvest), Rotation A
- Höllvania/Hell-Scrub: Scaldra (Survival), Rotation C
- Höllvania/Hell-Scrub: Techrot (Survival), Rotation C
- Ceres/Cinxia (Interception), Rotation C
- Earth/Gaia (Interception), Rotation C
- Mars/Alator (Interception), Rotation C
- Mercury/Odin (Interception), Rotation C
- Venus/Cytherean (Interception), Rotation C
- Europa/Paimon (Defense), Rotation A
- Jupiter/Io (Defense), Rotation A
- Saturn/Helene (Defense), Rotation A
- Sedna/Camenae (Defense), Rotation A
- Mars/Olympus (Disruption), Rotation C
- Sedna/Kappa (Disruption), Rotation A
- Uranus/Ur (Disruption), Rotation A
- Höllvania/Legacyte Harvest (Legacyte Harvest), Rotation C

