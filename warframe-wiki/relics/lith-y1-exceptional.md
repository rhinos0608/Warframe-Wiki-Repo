---
category: Relics
description: An artifact containing Orokin secrets. It can only be opened through
  the power of the Void.
image: ../images/lith-exceptional.png
last_updated: '2025-09-17'
name: Lith Y1 Exceptional
rewards:
- chance: 2
  item:
    name: Yareli Prime Neuroptics Blueprint
    uniqueName: /Lotus/Types/Game/Projections/T1VoidProjectionYareliPrimeEBronze
    warframeMarket:
      id: 682dfa9835715d4f3e9c64e8
      urlName: yareli_prime_neuroptics_blueprint
  rarity: Rare
- chance: 11
  item:
    name: Trumna Prime Stock
    uniqueName: /Lotus/Types/Game/Projections/T1VoidProjectionYareliPrimeEBronze
    warframeMarket:
      id: 673516c9db3ac2cfade14a7a
      urlName: trumna_prime_stock
  rarity: Uncommon
- chance: 11
  item:
    name: Zylok Prime Receiver
    uniqueName: /Lotus/Types/Game/Projections/T1VoidProjectionYareliPrimeEBronze
    warframeMarket:
      id: 6530694722c7fd977050854b
      urlName: zylok_prime_receiver
  rarity: Uncommon
- chance: 25.33
  item:
    name: Braton Prime Blueprint
    uniqueName: /Lotus/Types/Game/Projections/T1VoidProjectionYareliPrimeEBronze
    warframeMarket:
      id: 54a73e65e779893a797fff33
      urlName: braton_prime_blueprint
  rarity: Uncommon
- chance: 25.33
  item:
    name: Nautilus Prime Carapace
    uniqueName: /Lotus/Types/Game/Projections/T1VoidProjectionYareliPrimeEBronze
    warframeMarket:
      id: 66c60c9066bbb2aedca695ab
      urlName: nautilus_prime_carapace
  rarity: Uncommon
- chance: 25.33
  item:
    name: Lex Prime Barrel
    uniqueName: /Lotus/Types/Game/Projections/T1VoidProjectionYareliPrimeEBronze
    warframeMarket:
      id: 54a73e65e779893a797fff4d
      urlName: lex_prime_barrel
  rarity: Uncommon
source: WFCD
tags:
- Relic
- Relics
tradable: true
type: Relic
---

# Lith Y1 Exceptional

An artifact containing Orokin secrets. It can only be opened through the power of the Void.

## Relic Information

## Rewards

| Item | Rarity | Chance |
|------|---------|--------|
| Unknown | Rare | 2 |
| Unknown | Uncommon | 11 |
| Unknown | Uncommon | 11 |
| Unknown | Uncommon | 25.33 |
| Unknown | Uncommon | 25.33 |
| Unknown | Uncommon | 25.33 |

